//
//  StretchingOperator_Parametric.hpp
//  Elasticity
//
...
#ifndef StretchingOperator_Parametric_hpp
#define StretchingOperator_Parametric_hpp
...
/**
 * @brief Computes the stretching energy and its gradient for a monolayer or bilayer
 * @tparam tMesh Type of the mesh
 * @tparam MeshLayer The layer of the mesh we are computing for (single, bottom or top)
 *
 * @details
 * This class is responsible for computing the isotropic hyperelastic stretching energy of the current configuration
...
class StretchingOperator_Parametric : public EnergyOperator_DCS<tMesh>
{
public:
...
protected:
    ...
    /**
     * Computes the stretching energy and, if desired, the gradient
     *
...
public:
    ...
    StretchingOperator_Parametric(const MaterialProperties<tMaterialType> & material_properties):
    ...
    virtual void addEnergy(std::vector<std::pair<std::string, Real>> & out) const override
    {
        ...
        out.push_back(std::make_pair(prefix+"stretching_aa", lastEnergy));
    }
...
#endif /* StretchingOperator_Parametric_hpp */
